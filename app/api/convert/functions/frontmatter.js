...
//   // If frontmatter has a title, remove the duplicate H1 from the body.
//   // ReadMe pages often have both `title: X` in frontmatter AND `# X` as
//   // the first heading. Doc.AI uses the frontmatter title as the page title,
//   // so the H1 is redundant and should be stripped.
...
export function extractFrontmatter(mdxContent) {
  ...
  // If frontmatter has a title, remove the duplicate heading from the body.
  // ReadMe pages often have both `title: X` in frontmatter AND `# X` or `## X`
  // as the first heading. Doc.AI uses the frontmatter title as the page title,
  // so the heading is redundant and should be stripped.
  let cleanedBody = body;
  if (parsed.title) {
