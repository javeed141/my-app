...
//     const name = match[1];
//     if (!KNOWN_COMPONENTS.has(name) && !isInsideFence(fenceMap, match.index) && !isInsideJsxComment(cleanedContent, match.index)) {
...
// // ---------------------------------------------------------------
// // Check if a character index is inside a JSX comment ({/* ... */})
// // ---------------------------------------------------------------
// function isInsideJsxComment(content, charIdx) {
//   // Search backward from charIdx for the nearest {/* that isn't closed
//   let i = charIdx - 1;
//   while (i >= 2) {
//     // Found comment closer before any opener → not inside a comment
//     if (content[i] === "}" && content[i - 1] === "/" && content[i - 2] === "*") {
//       return false;
//     }
//     // Found comment opener → we're inside it
//     if (content[i] === "*" && content[i - 1] === "/" && i >= 2 && content[i - 2] === "{") {
//       return true;
//     }
//     i--;
//   }
...
//   const escaped = escapeRegex(name);
//   const tagPattern = new RegExp(`<${escaped}\\b`, "g");
//   let startMatch;

//   while ((startMatch = tagPattern.exec(content)) !== null) {
//     // Skip if inside a code fence or JSX comment
//     if (isInsideFence(fenceMap, startMatch.index)) continue;
//     if (isInsideJsxComment(content, startMatch.index)) continue;

//     const remaining = content.slice(startMatch.index);
//     const result = extractUsageFromRemaining(remaining, escaped);
...
// function extractUsageFromRemaining(remaining, escaped) {
