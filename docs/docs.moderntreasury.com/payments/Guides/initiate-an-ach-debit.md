...
An ACH debit allows you to receive, or pull, funds from an external bank account.  ACH debits have special risk considerations, including, for example, a much longer [return window](https://docs.moderntreasury.com/payments/docs/ach-return-risk-thresholds) than ACH credits.  To initiate an ACH debit, you will need to create a [Payment Order](/platform/reference/payment-order-object) with a [Counterparty](/platform/reference/counterparty-object).
...
When you create a Payment Order, you must specify which account you want the transaction to originate from. When originating an ACH debit this is the account that will receive money.
