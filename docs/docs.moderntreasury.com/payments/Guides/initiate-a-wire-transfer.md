...
Let's assume you want to send $10 using a wire transfer.  This could be paying a vendor, paying an employee salary, moving funds between corporate accounts, etc.  To do so, we will need to create a [Payment Order](/platform/reference/payment-order-object) with a [Counterparty](/platform/reference/counterparty-object).
...
> 📘 Sandbox Setup
>
> For more information on how we've set up your accounts, visit our [Sandbox Details](/payments/docs/payments-sandbox-overview) guide.

## 2. Create a Counterparty

Counterparties need different information depending on how you want to use them. In the case we want to send them money using a wire transfer, we need to ensure the customer has an account number, a `routing_number`, and an address.
