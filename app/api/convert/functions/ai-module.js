...
//   if (tasks.length === 0) {
//     console.log("[ai-module] No tasks for AI — all handled by known patterns or pre-filter");
//     return { conversions, warnings };
//   }

//   console.log(`[ai-module] Sending ${tasks.length} task(s) to AI:`, tasks.map(t => t.name));
...
//       console.log(`[ai-module] Task <${task.name}>: ${result.status}`, result.status === "rejected" ? result.reason?.message : "");
...
//   // Fix: Strip leading headings from AI output — component conversions
//   // replace a component IN PLACE and should never introduce new headings.
//   // The heading would break the page's hierarchy.
//   cleaned = cleaned.replace(/^#{1,6}\s+.+\r?\n*/gm, () => {
//     warnings.push(`Removed heading from <${componentName}> AI output — components should not add headings`);
//     return "";
//   });
